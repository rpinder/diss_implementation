...
let rec convert_mvar t pool env internal_seen term =
  ...
  | Ast.App (fi, e1, e2) -> Ast.App (fi, convert_mvar t pool env internal_seen e1, convert_mvar t pool env internal_seen e2)
  | Ast.Abs (fi, s, e1) ->
...
and force_obox t pool term =
  match term with
  | Ast.Nil -> Ast.Nil
  | Ast.OBox b -> eval t pool (Environment.create ()) b
  | Ast.Cons (fi, t1, t2) ->
...
and eval t pool env term = 
  ...
  | Ast.BinOp (_, t1, t2, op) ->
     ...
     (match (t1', t2') with
      ...
      | (x, y) -> eval t pool env (Ast.BinOp (Ast.empty_info, mvar_or_not t pool x, mvar_or_not t pool y, op)))
      (* | _ ->
